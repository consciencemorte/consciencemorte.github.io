...
On considère le flux résiduel à la couche (l),

$$
X^{(l)}\in\mathbb{R}^{T\times d_{\text{model}}}.
$$

Dans l’architecture Transformer standard, les sous-modules **lisent** une vue normalisée de l’état courant, puis **écrivent** une mise à jour additive dans le même espace résiduel. En notation pré-norme, on introduit

$$
\tilde X^{(l)}=\mathrm{Norm}!\big(X^{(l)}\big),
$$

et l’évolution de couche s’écrit comme une somme de mises à jour :

$$
X^{(l+1)} = X^{(l)} + \Delta X^{(l)}_{\mathrm{MHA}}\big(\tilde X^{(l)}\big) + \Delta X^{(l)}_{\mathrm{MLP}}\big(\tilde X^{(l)}\big).
$$

Ici, $\Delta X^{(l)}_{\mathrm{MHA}}$ provient d’un **routage inter-positions** (construction de poids d’attention via $Q/K$, puis transfert via $V$ et écriture via $O$), tandis que $\Delta X^{(l)}_{\mathrm{MLP}}$ provient d’une transformation **locale en position** (mélange de canaux au sein de chaque ligne $t$). Les sections suivantes détaillent la construction de $A^{(h)}(X$ sous masque causal et softmax, puis la lecture/écriture dans le résiduel via le circuit OV, avant d’aborder des motifs mécanistes inter-couches utiles pour l’analyse adversariale.

-> (revoir  ) masque causal et softmax, puis la décomposition du FFN comme opérateur d’écriture conditionnée dans le résiduel.
...
$\{A^{(h)}_{t,j}\}_{j\le t}$ sur les positions sources. Cette distribution induit un **mélange inter-positions** : la représentation en $t$ agrège une combinaison pondérée des valeurs portées par les positions passées

$$
\mathrm{head}^{(h)}_t = \sum_{j\le t} A^{(h)}_{t,j} v^{(h)}_j,
$$

où $v^{(h)}_j$ désigne la valeur projetée en tête $h$ à la position $j$.

Le mécanisme est **causal** : après application du masque causal, $A^{(h)}$ n’a de support que sur  $j\le t$, et vérifie
...
Il est **conditionné par l’état courant** : les poids $A^{(h)}_{t,j}$ dépendent des représentations à la couche $l$ via les requêtes en $t$ et les clés en $j$, et varient donc avec le contenu du contexte, plutôt que de réaliser un mélange à coefficients fixes. Autrement dit, l’attention implémente une famille d’opérateurs de mélange dont la **géométrie de routage** est déterminée par l’entrée elle-même.

La suite précise comment la distribution $A^{(h)}(X)$ est construite par le circuit QK (compatibilité bilinéaire, compétition softmax, contrainte causale et biais positionnels), puis comment les valeurs ainsi routées sont lues et réécrites dans le flux résiduel via le circuit OV, avant d’aborder des motifs mécanistes inter-couches (p. ex. têtes d’induction, puits d’attention) pertinents pour l’analyse adversariale.
