...
- $\iota : \mathcal{V}\rightarrow \{0,\dots,\vert\mathcal{V}\vert-1\}$ (étendue pointwise en $\iota^{*}:\mathcal V^{*}\to\{0,\dots,\vertl\mathcal V\vertr-1\}^{*}$) — **indexation** associant à chaque token un entier $i_k$ (Token ID).
...
Cette normalisation assure une somme unitaire. Les écarts linéaires de logits contrôlent les rapports de probabilité : une différence linéaire dans les scores se traduit par un rapport exponentiel dans les probabilités.
...
x_k^{(0)} = e_k + p_k,
\qquad
X^{(0)} \in \mathbb R^{T\times d_{\text{model}}}.
$$
...
Pour une séquence de longueur $T$, le flux résiduel est un tenseur
$$
X^{(l)} \in \mathbb{R}^{T \times d_{\text{model}}},
\qquad
x_t^{(l)} \in \mathbb{R}^{d_{\text{model}}}\ \text{(tranche à la position } t).
$$
...
Le flux résiduel ayant été établi comme un substrat additif (cf. §1.2), l’analyse porte sur les opérateurs qui (i) redistribuent l’information entre positions et (ii) transforment localement les représentations. Dans l’approche d’interprétabilité mécaniste, un bloc Transformer s’analyse comme l’alternance de deux mécanismes principalement distingués par l’axe tensoriel sur lequel ils agissent (Elhage et al., 2021) :

- Attention (mélange temporel / routage inter-positions). Opère sur l’axe de la séquence ($T$). À position cible fixée, elle agrège des composantes issues d’autres positions via des pondérations dépendantes du contenu. Après projections linéaires, son action peut être vue comme l’application d’une matrice d’attention $A(X)\in\mathbb{R}^{T\times T}$ (causale et ligne-stochastique après Softmax) au tenseur des valeurs :

  $$
  Q=XW_Q,\;\;K=XW_K,\;\;V=XW_V,\qquad
    A(X)=\mathrm{Softmax}\!\left(\frac{QK^\top}{\sqrt{d_k}}+M\right),
    \qquad
    \mathrm{Attn}(X)=A(X)VW_O,
  $$

  où $M$ encode le masque causal. Conditionnellement à $A(X)$, l’opérateur est linéaire en $V$ et réalise un mélange inter-positions.

- MLP/FFN (mélange de canaux / transformation locale). Opère sur l’axe des dimensions ($d_{\text{model}}$) à position fixée, indépendamment des autres tokens. Il implémente une application non linéaire pointwise :

  $$  \mathrm{MLP}(x)=W_2\,\phi(W_1x+b_1)+b_2,$$

  appliquée ligne par ligne à $X$. Ce mécanisme se prête à une lecture en mémoire associative (clé–valeur) au sens de Geva et al. (2021), écrivant dans le résiduel des directions latentes conditionnées par des motifs locaux.

Cette dichotomie structure l’analyse de sûreté : le mélange temporel détermine où et quand une information devient disponible (accessibilité causale), tandis que le mélange de canaux détermine comment cette information est compilée en variables latentes décodables (conversion en comportement, y compris conformité ou refus).

### Le bloc Transformer : deux écritures sur un même flux

Notations. $T$ désigne la longueur de séquence et $X^{(l)}\in\mathbb{R}^{T\times d_{\text{model}}}$ l’état à l’entrée du bloc $l$. Dans l’architecture Pre-Norm, le bloc s’exprime comme deux mises à jour additives successives via un état intermédiaire $X'^{(l)}$ :

$$
\begin{aligned}
\Delta X^{(l)}_{\mathrm{MHA}} &:= \mathrm{MHA}\!\left(\mathrm{Norm}_1(X^{(l)})\right), \\
X'^{(l)} &= X^{(l)} + \Delta X^{(l)}_{\mathrm{MHA}}, \\
\Delta X^{(l)}_{\mathrm{MLP}} &:= \mathrm{MLP}\!\left(\mathrm{Norm}_2(X'^{(l)})\right), \\
X^{(l+1)} &= X'^{(l)} + \Delta X^{(l)}_{\mathrm{MLP}}.
\end{aligned}
$$

L’attention et le MLP ne constituent pas deux couches substitutives, mais deux opérateurs d’écriture qui injectent des mises à jour $\Delta X$ dans un espace vectoriel partagé (le flux résiduel). La lecture mécaniste consiste alors à attribuer, au sein de la somme des contributions, l’émergence de composantes directionnelles qui, une fois décodées par $W_U$, modulent la distribution de sortie (cf. §1.2).
<hr style="width:40%; margin:auto;">
